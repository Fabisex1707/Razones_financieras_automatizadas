            ...
            if isinstance(estado_resultados, dict) and isinstance(balance_general, dict):
                while True:
                    ...
                    if opcion == 7:
                        ...
                        AC=int(input("Activo Circulante: "));
                        PCP=int(input("Pasivo Corto Plazo: "));
                        Inv=int(input("Inventario: "));
                        E=int(input("Efectivo: "));
                        CTN=AC-PCP;
                        RC=AC/PCP;
                        PA=(AC-Inv)/PCP;
                        RE=E/PCP;
                        print(f"Los resultados son: \nCapital de Trabajo Neto: {CTN}\nRazon Circulante: {RC}\nPrueba Acida: {PA}\nRazon de Efectivo: {RE}")   
                    elif opcion == 3:
                        Pasi=int(input("Pasivos: "));
                        Actv=int(input("Activos: "));
                        CapCont=int(input("Capital Contable: "));
                        PasLP=int(input("Pasivo Largo Plazo: "));
                        CapSoc=int(input("Capital Social: "));
                        UAI=int(input("Utilidad Antes de Impuestos: "));
                        GF=int(input("Gastos Financieros: "));
                        REnd=Pasi/Actv;
                        RCap=CapCont/Actv;
                        AF=PasLP/CapSoc;
                        CI=(UAI+GF)/GF;
                        print(f"Los resultados son: \nRazon de Endeudamiento: {REnd}\nRazon Capital: {RCap}\nApalancamiento Financiero: {AF}\nCobertura de Interno: {CI}")   
                    elif opcion == 4:
